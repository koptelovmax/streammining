...
w = 1000 # size of the window
f = 100 #? number of examples between checks for drift


# Initialization of parameters:
ALT = [] # empty set of alternate trees for l1
W = [] # empty window of examples
L = [] # set of leaves
candAttrib = [] # list of candidate attributes for splitting the root
for i in range(len(givenAttrib)):
    candAttrib.append(HNode(i,givenAttrib[i],c))

# CVFDT initialization:    
HT = Hleaf(c,candAttrib) # tree with single leaf (root) l1
L.append('l1') # add leaf l1 (root) to the set of leaves

# CVFDT start:
ID = 0 # max ID of leaves in L
        ...
        if count < 1000000:
            try:
                ...
                try:
                    ...
                    for tree in ALT:
                        _ = tree.sortExampleAndPredict(example,text_file)
                    
                    ID = len(L)-1
                    W.insert(0,[example,ID]) # add ((x,y),ID) to the beginning of W
                    
                    if len(W) > w:
                        # TODO: ForgetExamples(HT,example,...)
                        #ForgetExamples(HT,n,W[-1])
                        _ = W.pop(-1) # remove last element
                        
                        # TODO: check CVFDTGrow
                        
                        if np.mod(count,f)==0:
                            # TODO: CheckSplitValidity(HT,n,DELTA)
